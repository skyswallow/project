...
Validator.messageSource['en-us'] = [
    ...
    ['validate-currency-dollar' , '请输入有效的货币类型. 如 100.00 .'],
...
Validator.messageSource['zh-cn'] = [
    ...
    ['validate-alphanum' , '请输入英文字母、数字或者下划线,其它字符是不允许的.'],
    ...
    ['validate-phone','请输入正确的电话号码,如:01029392929,当前长度为%s.'],
    ['validate-mobile-phone','请输入正确的手机号码,当前长度为%s.'],
    ['validate-fax','请输入正确的传真号码'],
...
Validation.prototype = {
    ...
    validate : function() {
        ...
        if(this.options.stopOnFirst) {
            ...
            for(var i = 0; i < elements.length; i++) {
                var elm = elements[i];
                result = Validation.validateElement(elm,{
                    useTitle : useTitles,
                    onElementValidate : callback
                });
                if(!result) break;
            ...
            for(var i = 0; i < elements.length; i++) {
                var elm = elements[i];
                if(!Validation.validateElement(elm,{
                    useTitle : useTitles,
                    onElementValidate : callback
                })) {
                    result = false;
        ...
        if(!result && this.options.focusOnError) {
            var first = Form.getElements(this.form).findAll(function(elm){
                return $prototype(elm).hasClassName('validation-failed')
            })[0];
...
Validation.addAllThese([
    ...
    ['validate-fax',/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})$/],
    ['validate-mobile-phone',/^(13[0-9]|15[0-9]|18[8|9])\d{8}$/],
